...
module.exports = function container (get, set, clear) {
  return {
    ...
    calculatePrice(signal, mid_price, markup_pct, increment) {
      if (signal === 'buy') {
        return n(mid_price).add(n(mid_price).multiply(markup_pct / 100)).format(increment, Math.ceil);
      } else if (signal === 'sell') {
        return n(mid_price).subtract(n(mid_price).multiply(markup_pct / 100)).format(increment, Math.floor);
      }
    },

    shouldCancel(signal, orderPrice, markedPrice) {
      if (signal === 'buy') {
        return (orderPrice > markedPrice);
      } else if (signal === 'sell') {
        return (orderPrice < markedPrice);
      }
    },
