...
return {
  name: 'crossed_ema',
  description: 'Buy when (fast_ema - slow_ema > 0) and sell when (fast_ema - slow_ema < 0). Optional buy on low RSI.',

  getOptions: function () {
    this.option('period', 'period length', String, '2m')
    this.option('min_periods', 'min. number of history periods', Number, 52)
    this.option('fast_ema', 'number of periods for fast_ema', Number, 12)
    this.option('slow_ema', 'number of periods for slow_ema', Number, 26)
  ...
  calculate: function (s) {
    s.cancel_down = false
    get('lib.ema')(s, 'fast_ema', s.options.fast_ema)
    get('lib.ema')(s, 'slow_ema', s.options.slow_ema)

    // console.log(s.period.fast_ema + ' | ' + s.period.slow_ema)
    ...
    if (s.period.fast_ema && s.period.slow_ema) {
      s.period.crossed_ema_rate = (s.period.fast_ema - s.period.slow_ema) / s.period.slow_ema * 100
      // console.log(s.period.crossed_ema_rate)
    }
    ...
    if (s.options.neutral_rate === 'auto') {
      get('lib.stddev')(s, 'crossed_ema_stddev', 10, 'crossed_ema_rate')
    }
    else {
      s.period.crossed_ema_stddev = s.options.neutral_rate
    }
  },
  ...
  onPeriod: function (s, cb) {
    ...
    if (s.period.crossed_ema_rate) {
      if (s.period.crossed_ema_rate > 0) {
        if (s.trend !== 'up') {
      ...
      else if (!s.cancel_down && s.period.crossed_ema_rate < 0) {
        // console.log(s.trend + ' | ' + s.acted_on_trend)
        if (s.trend !== 'down') {
          s.acted_on_trend = false
        }
        s.trend = 'down'
    ...
    // console.log(s.period.crossed_ema_rate + ' -> ' + s.trend + ' -> ' + s.signal)
  ...
  onReport: function (s) {
    ...
    if (typeof s.period.crossed_ema_stddev === 'number') {
      var color = 'grey'
      if (s.period.crossed_ema_rate > s.period.crossed_ema_stddev) {
        color = 'green'
      }
      else if (s.period.crossed_ema_rate < (s.period.crossed_ema_stddev * -1)) {
        color = 'red'
      }
      cols.push(z(8, n(s.period.crossed_ema_rate).format('0.0000'), ' ')[color])
      if (s.period.crossed_ema_stddev) {
        cols.push(z(8, n(s.period.crossed_ema_stddev).format('0.0000'), ' ').grey)
      }
    }
    else {
      if (s.period.crossed_ema_stddev) {
