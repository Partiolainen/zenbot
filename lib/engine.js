    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      ...
      syncBalance(function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy' && (so.buy_when_asset_less >= s.balance.asset || (typeof so.buy_when_asset_less!='number')
                          || so.buy_when_asset_less === 0)) {
            ...
            if (!size) {
              if (so.mode === 'live') {
                if (so.order_type === 'maker') {
                  size = n(Math.min(s.balance.currency,so.buy_abs_max)).multiply(so.buy_pct).divide(100).multiply(s.exchange.makerFee / 100).format('0.00000000');
                } else {
                  size = n(Math.min(s.balance.currency,so.buy_abs_max)).multiply(so.buy_pct).divide(100).multiply(s.exchange.takerFee / 100).format('0.00000000');
                }
                size = n(Math.min(s.balance.currency,so.buy_abs_max)).multiply(so.buy_pct).divide(100).subtract(size).divide(price).format('0.00000000')
              } else {
                size = n(Math.min(s.balance.currency,so.buy_abs_max)).multiply(so.buy_pct).divide(100).divide(price).format('0.00000000')
              }
            }
